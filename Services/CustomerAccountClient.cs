...
{
    Task<AccountResult<AccountCodeChallenge>> RegisterAsync(string email, string name, string phone, string password, bool acceptedPrivacyPolicy, string privacyPolicyVersion, CancellationToken cancellationToken = default);
    Task<AccountResult<(CustomerAccountSession Session, CustomerAccountProfile Profile)>> VerifyEmailAsync(Guid challengeId, string code, bool acceptedPrivacyPolicy, string privacyPolicyVersion, CancellationToken cancellationToken = default);
    Task<AccountResult<(CustomerAccountSession Session, CustomerAccountProfile Profile)>> LoginAsync(string email, string password, CancellationToken cancellationToken = default);
    ...
    Task<AccountResult<bool>> CloseAsync(string token, string currentPassword, CancellationToken cancellationToken = default);
    ...
    IHostEnvironment environment) : ICustomerAccountClient
{
    ...
    public Task<AccountResult<AccountCodeChallenge>> RegisterAsync(string email, string name, string phone, string password, bool acceptedPrivacyPolicy, string privacyPolicyVersion, CancellationToken ct = default) => SendAsync<AccountCodeChallenge>(HttpMethod.Post, "v1/storefront/account/auth/register", new { email, name, phone, password, acceptedPrivacyPolicy, privacyPolicyVersion }, null, ct);
    public async Task<AccountResult<(CustomerAccountSession Session, CustomerAccountProfile Profile)>> VerifyEmailAsync(Guid challengeId, string code, bool acceptedPrivacyPolicy, string privacyPolicyVersion, CancellationToken ct = default)
    {
        var result = await SendAsync<SessionDto>(HttpMethod.Post, "v1/storefront/account/auth/verify-email", new { challengeId, code, acceptedPrivacyPolicy, privacyPolicyVersion }, null, ct);
    ...
    public async Task<AccountResult<(CustomerAccountSession Session, CustomerAccountProfile Profile)>> LoginAsync(string email, string password, CancellationToken ct = default)
    {
        var result = await SendAsync<SessionDto>(HttpMethod.Post, "v1/storefront/account/auth/login", new { email, password }, null, ct);
        if (result.State != AccountLoadState.Success || result.Value is null)
            return AccountResult<(CustomerAccountSession, CustomerAccountProfile)>.Failure(result.State, result.Message);

        return new(result.State, (new(result.Value.SessionToken, timeProvider.GetUtcNow(), result.Value.ExpiresAt), Map(result.Value.Profile)), result.Message);
    }
    public Task<AccountResult<AccountCodeChallenge>> RequestPasswordCodeAsync(string? token, string email, CancellationToken ct = default) => SendAsync<AccountCodeChallenge>(HttpMethod.Post, "v1/storefront/account/auth/password/request-code", new { email }, token, ct);
    public async Task<AccountResult<(CustomerAccountSession Session, CustomerAccountProfile Profile)>> ResetPasswordAsync(string? token, Guid challengeId, string code, string password, CancellationToken ct = default)
    {
        var result = await SendAsync<SessionDto>(HttpMethod.Post, "v1/storefront/account/auth/password/reset", new { challengeId, code, password }, token, ct);
        if (result.State != AccountLoadState.Success || result.Value is null)
            return AccountResult<(CustomerAccountSession, CustomerAccountProfile)>.Failure(result.State, result.Message);
    ...
    public Task<AccountResult<bool>> CloseAsync(string token, string currentPassword, CancellationToken ct = default) => NoContentAsync(HttpMethod.Post, "v1/storefront/account/close", new { currentPassword }, token, ct);
        ...
        try
        {
            ...
            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                if (string.IsNullOrWhiteSpace(token))
                    return AccountResult<T>.Failure(AccountLoadState.Unauthorized, "E-mail ou senha inválidos.");
                cookies.Clear();
