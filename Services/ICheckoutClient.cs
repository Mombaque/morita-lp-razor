...
public interface ICheckoutClient
{
    ...
    Task<PaymentResult> InitiateCardAsync(Guid publicCheckoutId, string accessToken, string idempotencyKey, string paymentToken, CancellationToken cancellationToken = default) => Task.FromResult(PaymentResult.Failure(PaymentLoadState.Unavailable));
