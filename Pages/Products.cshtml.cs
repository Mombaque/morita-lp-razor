...
public class ProductsModel(ICatalogClient client) : PageModel
{
    ...
    public async Task OnGetAsync(int? page, CancellationToken cancellationToken)
    {
        ...
        var query = new CatalogQuery(Search, CategoryId, ModalityId, BrandId, SizeId, ColorId, Available, CurrentPage, Sort);
    ...
    private Dictionary<string, string?> QueryValues(int page) => new Dictionary<string, string?>
    {
        ...
        ["sizeId"] = SizeId?.ToString(), ["colorId"] = ColorId?.ToString(), ["available"] = Available?.ToString().ToLowerInvariant(), ["sort"] = Sort, ["page"] = page.ToString()
